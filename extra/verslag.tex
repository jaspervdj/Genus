...
\subsection{Cykels sneller sluiten}
...
\section{Cykels die we kunnen splitsen}
Het feit dat we soms cykels kunnen splitsen (zie \ref{sluit-pad-vroeger}) kunnen
we ook gebruiken als bounding criterium. Met andere woorden, als ons pad langs
een top gaat waar we al eerder doorgingen, zal de uiteindelijke cykel gesplitst
kunnen worden in twee cykels. Onze embedding zal dus geen optimaal aantal
vlakken hebben, en we mogen dus bounden.
\newline

Hoe detecteren we nu dat we ergens al gepasseerd zijn? De beste manier is om
elke boog een label te geven, en zo bij te houden in welk pad een bepaalde boog
ligt.
